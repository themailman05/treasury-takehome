...
- **Visual audit overlay.** The single-label result renders the uploaded image with a labelled, colour-coded box per region — green pass / red fail / amber review — so a non-technical agent sees *where* a defect is at a glance. Field boxes come from the VLM (normalized 0–1000 coords); the **warning box is OCR-located** (the union of the anchored warning words) because the model rarely returns a usable warning box — that keeps the warning box on the real warning across layouts instead of a guessed region.
