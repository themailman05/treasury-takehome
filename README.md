...
- **Finding + reading the warning (layout-robust).** `TesseractOCR` upscales the label, locates the `GOVERNMENT` **text anchor** (`image_to_data`), computes the warning's **bounding box** (the anchored words in that column), then **crops tightly to that box and upscales it** before the final read — so it reads the warning wherever it sits (bottom of a portrait label *or* mid-back-panel on a front+back layout) and reads it *cleanly* (tight-crop+upscale lifted a noisy back-panel warning from 78% → 100% clause match → `pass`). The box is also returned for the audit overlay. Falls back to the bottom of the label if the header is too degraded to anchor.
- **Warning matching: similarity bands, not an edit budget.** Full-page OCR mis-segments the small warning text, hence the anchored crop above. We first used a per-character edit *budget*, but label-dependent OCR noise (8–24 edits even on compliant labels) put it on a tightrope. The fix: judge each clause by **fuzzy similarity** — ≥85% present, <60% missing/reworded, in between → review; header casing from the case-preserving OCR. Robust to OCR noise, still catches missing/absent/reworded/title-case. (Inherent limit: a *subtle* single-word reword can't be distinguished from OCR noise — documented; caught only at lower confidence → review.)
- **Visual audit overlay.** The single-label result renders the uploaded image with a labelled, colour-coded box per region — green pass / red fail / amber review — so a non-technical agent sees *where* a defect is at a glance. **All boxes are OCR-grounded**: in one `image_to_data` pass we locate the warning by its anchor and locate each extracted field's text among the OCR words, boxing it where it actually appears. The VLM's own bounding boxes proved unreliable on complex layouts (it put ABV/net-contents on a cash-refund blurb), so we don't use them. A field whose text can't be confidently located gets **no box** (omitted) rather than a wrong one.
...
- **Real-world labels are harder than clean test art — two prompt bugs fixed, one real limit remains.** Testing on actual product labels surfaced two prompt issues (now fixed): the model would (a) **echo the prompt's example values** (`45% Alc./Vol.`, `750 mL`) when a label was hard to read, and (b) **ramble/hallucinate the warning** when asked to transcribe it, blowing the token cap → truncated JSON. The prompt now carries **no example values** and asks the model only to *locate* the warning (Tesseract reads the text). The remaining gap is genuine: real-world accuracy is bounded by **input resolution** (a 375 px thumbnail's small ABV/class text is near-unreadable, so the model defaults to a typical `40%/80 proof`) and the off-the-shelf model on **varied layouts** (landscape labels put the warning in a corner the bottom-crop can't isolate). Closing it is the §10 work: require higher-res capture, **box-guided per-field re-OCR**, PaddleOCR, and **fine-tuning on real labels**. The system degrades safely throughout — a hard label is `needs_review`, never a wrong `pass` or a 5xx.
...
**Measured:** ~20s per label end-to-end on the live demo (warm). Almost all of it is the **Gemma 4 vision call** (image prefill + ~25 tok/s decode); Tesseract OCR of the warning is ~1s, and the deterministic verification (rapidfuzz + the warning comparator) is sub-millisecond. So latency is gated entirely by the model call — the verification logic itself is effectively free. (The **audit bounding boxes** add ~7s of output tokens; without them a label is ~13s — the boxes are an accuracy/UX-vs-latency trade-off you can drop if speed matters more.)

**Why we're slow: the GPU.** The demo runs on an **NVIDIA L4** — Nvidia's *entry-level, economical* inference card (24 GB, ~300 GB/s memory bandwidth). LLM token decode is memory-bandwidth-bound, so a 12B multimodal model is slow on an L4 relative to data-center GPUs. We chose the L4 for cost, not speed.

**Two robustness fixes from real-world testing:** (1) the model is kept **resident** (`keep_alive=-1`) so a label after an idle gap doesn't pay a ~30s cold reload; (2) if a hard photo makes the model run past the token cap (truncated/invalid JSON) or the backend errors, the pipeline **degrades to `needs_review:unreadable`** rather than returning a 502 — one bad photo never errors the request or stalls a batch.
