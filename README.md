...
- **Finding the warning (layout-robust).** `TesseractOCR` upscales the label, locates the `GOVERNMENT` **text anchor** (`image_to_data`), and crops from there — so it reads the warning wherever it sits (bottom of a portrait label *or* the middle of a back panel on a front+back layout; a fixed bottom-crop missed the latter). Falls back to the bottom of the label if the header is too degraded to anchor.
