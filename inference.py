...
# (see verify_warning).
EXTRACTION_PROMPT = (
    "You are a meticulous OCR transcription engine for U.S. alcohol-beverage "
    "(TTB COLA) label artwork. Transcribe what is PRINTED ON THE LABEL, exactly "
    "and verbatim.\n"
    "\n"
    "Hard rules:\n"
    "- Transcribe character-for-character. Do NOT correct spelling, casing, "
    "spacing, or punctuation. Do NOT complete or 'fix' partial or misspelled "
    "text. If a word is misspelled on the label, copy the misspelling.\n"
    "- Do NOT supply text from memory. In particular, never reproduce the "
    "canonical government health warning from prior knowledge — transcribe only "
    "what is actually visible, including any defects, omissions, or wrong casing.\n"
    "- If a field is not present on the label, return an empty string for its "
    "text and confidence 0.\n"
    "\n"
    "Extract these fields:\n"
    "- brand_name: the brand / product name as printed.\n"
    "- class_type: the class or type designation (e.g. 'Kentucky Straight "
    "Bourbon Whiskey').\n"
    "- abv: the alcohol-content statement verbatim (e.g. '45% Alc./Vol. "
    "(90 Proof)').\n"
    "- net_contents: the net contents statement verbatim (e.g. '750 mL').\n"
    "- warning: the government health warning region. Set 'vlm_text' to your "
    "verbatim transcription of that region, and 'located' true if you can find it "
    "(else false). Leave 'ocr_text' null — a separate deterministic OCR step fills it.\n"
    "- image_quality: 'ok' if the label is clearly readable, 'poor' if readable "
    "but degraded (glare/angle/blur), 'unreadable' if you cannot reliably read "
    "the label at all.\n"
    "\n"
    "BOUNDING BOXES: set 'box' = [ymin, xmin, ymax, xmax] normalized to 0-1000 "
    "(top-left origin) for each region you locate — on brand_name, class_type, "
    "abv, net_contents, AND inside the warning object (set warning.box to the "
    "government-warning region's box). Use null for a region that is not present.\n"
    "\n"
    "Each text field carries a 'confidence' in [0, 1] reflecting how certain you "
    "are of the transcription.\n"
    "\n"
